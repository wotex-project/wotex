...
# WMA.01 Software implementation rules
...
Read this file with [WMA.05](WMA.05-software-contract.md),
[standalone client preservation](WMA.06-standalone-client-and-preservation.md),
[Wotex integration](WMA.07-wotex-integration.md), and the existing
...
## WMA-C01 — Scope, API and compatibility

Build the explicitly enumerated client profile in WMA.05. A protocol's
...
## WMA-C02 — Input validation and bounds
...
## WMA-C03 — Session ownership and deadlines
...
## WMA-C04 — Errors, effects and retry

Keep `%Wotex.Matter.Error{}` with fields `code`, `field`, `details`,
...
## WMA-C05 — Subscription contract

Applies only to subscriptions listed in WMA.05. Native `subscribe/2` takes a
validated request map with `receiver: pid` (default caller) and returns
`{:ok, %Wotex.Matter.Subscription{pid: pid, reference: ref, generation: gen}}` only after
...
Native deliveries are `{:wotex_matter, reference, {:ok, value, metadata}}` or
`{:wotex_matter, reference, {:error, error}}`. An initial value is
...
## WMA-C06 — Runtime and Form mapping
...
types specified in WMA.05. A credential/profile mismatch fails before I/O.
...
Protocol-specific parameters and value envelopes are defined in WMA.05.
...
## WMA-C08 — Telemetry and diagnostics

Retain request event `[:wotex, :matter, :request, :stop]`, duration in native
monotonic units and finite operation/result/status metadata. Add stream events
`[:wotex, :matter, :subscription, :open | :deliver | :close]` with counts and a
...
Each requirement ID in WMA.05/.11/.12/.13 requires concrete acceptance cases with
...
## WMA-C10 — Commits and completion
...
`mix pkg wotex-matter check --no-retry`, when the package changed. Preserve the
...
source, the package gate `mix pkg wotex-matter check --no-retry` passes, including
...
exploratory patches in the ignored `docs/tasks/local/wotex-matter/`.
