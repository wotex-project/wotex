...
| WBM-P05 | Dependency resolution checks the repository lock without mutation, while the external archive consumer creates and rechecks an independent lock before its behavioral test pass |
...
| WBM-P07 | `.tool-versions` declares Elixir `1.18.4-otp-27` with OTP `27.3.4.15`; each executed compatibility lane records its actual runtime and dependency revisions |
| WBM-P08 | The developer gate remains compile, format, and behavioral tests; audits, documentation, coverage, Dialyzer, application, boundary, archive, and diff checks run as explicit evidence |
