# Wotex Thread completion contract
...
obligation. Documentation now lives under `docs/packages/wotex-thread/`.
...
(`WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-thread`) and the
CI lanes now discharge `repository_green` and `archive_consumer_green`. Tags
use `wotex-thread-v<version>`.
...
Mutable audit notes remain in the ignored root `docs/tasks/local/wotex-thread/`;
