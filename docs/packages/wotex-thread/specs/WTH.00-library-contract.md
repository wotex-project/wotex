...
# WTH.00 Software implementation rules
...
Read this file with [WTH.10](WTH.10-software-contract.md),
[standalone client preservation](WTH.11-standalone-client-and-preservation.md),
[Wotex integration](WTH.12-wotex-integration.md), and the existing
...
## WTH-C01 — Scope, API and compatibility

Build the explicitly enumerated client profile in WTH.10. A protocol's
...
## WTH-C02 — Input validation and bounds
...
## WTH-C03 — Session ownership and deadlines
...
## WTH-C04 — Errors, effects and retry

Keep `%Wotex.Thread.Error{}` with fields `code`, `field`, `details`,
...
## WTH-C05 — Subscription contract

Applies only to subscriptions listed in WTH.10. Native `subscribe/2` takes a
validated request map with `receiver: pid` (default caller) and returns
`{:ok, %Wotex.Thread.Subscription{pid: pid, reference: ref, generation: gen}}` only after
...
Native deliveries are `{:wotex_thread, reference, {:ok, value, metadata}}` or
`{:wotex_thread, reference, {:error, error}}`. An initial value is
...
## WTH-C06 — Runtime and Form mapping
...
types specified in WTH.10. A credential/profile mismatch fails before I/O.
...
## WTH-C07 — Native executable contract
...
Protocol-specific parameters and value envelopes are defined in WTH.10.
...
## WTH-C08 — Telemetry and diagnostics

Retain request event `[:wotex, :thread, :request, :stop]`, duration in native
monotonic units and finite operation/result/status metadata. Add stream events
`[:wotex, :thread, :subscription, :open | :deliver | :close]` with counts and a
...
## WTH-C09 — Mandatory software evidence

Each requirement ID in WTH.10/.11/.12/.13 requires concrete acceptance cases with
...
`mix pkg wotex-thread check --no-retry`, when the package changed. Preserve the
