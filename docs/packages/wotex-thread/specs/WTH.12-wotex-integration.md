...
# WTH.12 Wotex integration and evidence contract

This is an accepted **target specification**, not implemented-profile evidence.
It makes [.10](WTH.10-software-contract.md) and
[.11](WTH.11-standalone-client-and-preservation.md) usable with the public Wotex
...
## WTH-I02 — Explicit Runtime profile factory

Add pure `profile/0` on `Wotex.Thread`, returning a `Wotex.Runtime.BindingProfile` for
`:daemon`. Add `profile/1`, accepting only the atoms below and returning
...
| --- | --- | --- | --- | --- |
| `:daemon` | `:thread` | `thread+unix` | readproperty | none |
...
Runtime exposes only the four local management inspection paths. Native SDK state subscriptions do not widen this profile. A consumer may explicitly compose Thread networking with CoAP or Matter; this library does not manufacture application Property/Action semantics.
...
{:ok, consumed} = Wotex.Runtime.ConsumedThing.new(td,
  ...
  transports: %{Wotex.Runtime.BindingProfile.id(profile) =>
    {Wotex.Thread.Transport, transport_options}},
...
## WTH-I03 — Mapping, route and value boundary
...
The controller-key must exactly equal config.target; only the configured daemon socket and four allowed paths are reachable.
...
## WTH-I04 — Errors, credentials and deadlines through Runtime
...
## WTH-I05 — Runtime stream ownership
...
native `{:wotex_thread, reference, event}` messages to Runtime and expect decoding.
...
The native implementation and bounded callback flow follow [WTH.13](WTH.13-native-backend.md). Runtime owns the final subscription process; an
