...
Read this file with [WTH.04](WTH.04-software-contract.md),
...
Build the explicitly enumerated client profile in WTH.04. A protocol's
...
Applies only to subscriptions listed in WTH.04. Native `subscribe/2` takes a
...
types specified in WTH.04. A credential/profile mismatch fails before I/O.
...
Protocol-specific parameters and value envelopes are defined in WTH.04.
...
Each requirement ID in WTH.04/.11/.12/.13 requires concrete acceptance cases with
