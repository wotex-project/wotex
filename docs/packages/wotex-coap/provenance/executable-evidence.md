...
OTP 29 and once on Elixir 1.18.4 / OTP 27, from commit `160bcc38`. Each lane
builds libcoap with the ten ordered patches and compiles the 12 native
fault/vector executables with ASan/UBSan, including the production-worker
exchange harness, and all exit 0; each software run passes all 57 tests,
including the independent Californium cohort on OpenJDK 17 and the peer guardian
tests, and measures no retained peer process. The production helper and peer are
byte-identical across lanes. Earlier lanes found the OTP 27 archive link failure
fixed in `d8d856f` and the exit-cancellation race fixed in `40c9c0f7`. The helper
that ExUnit drives in these lanes is not itself sanitizer-instrumented; its
sanitized evidence remains the `test/native/Dockerfile` harness lane.
...
peer instance and one fresh client context. The renewed software-run receipt
includes this cohort on macOS with OpenJDK 21, and the Linux receipt on both
runtimes with OpenJDK 17; the Java
peer is a test peer only and is neither a runtime nor an orchestration
dependency of the package.

The intermittent PKI Runtime unary timeout in `test/interop/dtls_pki_test.exs`
is not reproduced and its cause is not established. Two macOS software runs on
2026-09-17 failed one configured-custody PKI call each (`invokeaction`, then
`writeproperty` with a trial 5,000 ms budget) with the CoAP exchange error
`timeout` after a successful handshake, at a load average of 11 to 13, while
earlier interrupted runs had left several hundred `coap-server` processes
running and container lanes shared the host. No source on that path has
changed since. On 2026-09-19 the unary matrix, the complete file and the full
software cohort passed in 28 runs at load averages from 9 to 48, generated by
busy loops on the host, by busy loops inside the container VM, and with 600
idle `coap-server` processes present. Under twice as many busy loops as cores,
324 timed calls had a median of 4.6 ms, a 99th percentile of 77 ms and a
