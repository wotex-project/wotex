...
# WCO.12 Wotex integration and evidence contract

This is an accepted **target specification**, not implemented-profile evidence.
It makes [.10](WCO.10-software-contract.md) and
[.11](WCO.11-standalone-client-and-preservation.md) usable with the public Wotex
...
## WCO-I02 — Explicit Runtime profile factory

Add pure `profile/0` on `Wotex.CoAP`, returning a `Wotex.Runtime.BindingProfile` for
`:udp`. Add `profile/1`, accepting only the atoms below and returning
...
| --- | --- | --- | --- | --- |
| `:udp` | `:coap` | `coap` | readproperty, writeproperty, invokeaction | none |
| `:udp_observe` | `:coap_observe` | `coap` | readproperty, writeproperty, invokeaction, observeproperty, unobserveproperty, subscribeevent, unsubscribeevent | Property representations / Event representations |
| `:dtls` | `:coaps` | `coaps` | readproperty, writeproperty, invokeaction, observeproperty, unobserveproperty, subscribeevent, unsubscribeevent | same representations through DTLS |
| `:oscore` | `:coap_oscore` | `coap` | readproperty, writeproperty, invokeaction, observeproperty, unobserveproperty, subscribeevent, unsubscribeevent | same representations through OSCORE |

The baseline profile has `media_types: ["application/json", "application/octet-stream", "text/plain;charset=utf-8"]`. Only the three listed representation codecs are exposed. Mapping validates media parameters, Accept and Content-Format together; it cannot select arbitrary payload codecs. Empty body, JSON null, empty bytes and malformed JSON remain distinct outcomes as required by .10.
...
Observe Event payloads are fresh resource representations, not a lossless server event log. A UDP Form cannot select DTLS. OSCORE is an explicit profile/configuration choice on coap; no security is inferred from that URI scheme alone.
...
## WCO-I03 — Mapping, route and value boundary
...
The parsed URI endpoint is the explicit route; resolving a discovered URI never opens a connection.
...
## WCO-I04 — Errors, credentials and deadlines through Runtime
...
## WCO-I05 — Runtime stream ownership
...
native `{:wotex_coap, reference, event}` messages to Runtime and expect decoding.
...
`runtime_read` projects CoAP codes/options to method, path and Accept.
...
The classifier-only invalid_header case tests propagation of an admitted codec
failure. It does not redefine how unrelated/malformed UDP traffic is handled:
S01 still ignores or rejects such traffic without spuriously completing another
exchange. Exercise that actual exchange behavior in its separate wire tests.

Native execution and Mix/ExUnit software ownership follow
[WCO.13](WCO.13-native-build-and-software-evidence.md); protocol-specific security selection
