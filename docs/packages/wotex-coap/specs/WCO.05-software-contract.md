...
# WCO.05 Complete CoAP client software profile

Read [WCO.01](WCO.01-library-contract.md) first. Required software includes bounded
...
Native OSCORE, its Runtime admission and the independent secure matrix execute
through the manifest-bound helper and pinned upstream peer.
[WCO.03](WCO.03-implemented-profile.md) defines supported behavior;
[WCO.08](WCO.08-native-build-and-software-evidence.md) fixes native build, Port
...
The [standalone/preservation contract](WCO.06-standalone-client-and-preservation.md)
...
Keep the WCO.04 atomic profile: 1 MiB body ceiling, default block size 512,
