...
Read this file with [WCO.10](WCO.10-software-contract.md),
...
Build the explicitly enumerated client profile in WCO.10. A protocol's
...
Applies only to subscriptions listed in WCO.10. Native `subscribe/2` takes a
...
types specified in WCO.10. A credential/profile mismatch fails before I/O.
...
Protocol-specific parameters and value envelopes are defined in WCO.10 and
[WCO.13](WCO.13-native-build-and-software-evidence.md). Native protocols execute in BEAM/OTP or
...
[WCO.13](WCO.13-native-build-and-software-evidence.md) fixes the explicit Mix task, native
...
Each requirement ID in WCO.10/.11/.12 requires concrete acceptance cases with
