...
Read this file with [WCO.05](WCO.05-software-contract.md),
[standalone client preservation](WCO.06-standalone-client-and-preservation.md),
[Wotex integration](WCO.07-wotex-integration.md), and the existing
...
Build the explicitly enumerated client profile in WCO.05. A protocol's
...
## WCO-C02 — Input validation and bounds
...
## WCO-C03 — Session ownership and deadlines
...
## WCO-C04 — Errors, effects and retry

Keep `%Wotex.CoAP.Error{}` with fields `code`, `field`, `details`,
...
## WCO-C05 — Subscription contract

Applies only to subscriptions listed in WCO.05. Native `subscribe/2` takes a
validated request map with `receiver: pid` (default caller) and returns
`{:ok, %Wotex.CoAP.Subscription{pid: pid, reference: ref, generation: gen}}` only after
...
Native deliveries are `{:wotex_coap, reference, {:ok, value, metadata}}` or
`{:wotex_coap, reference, {:error, error}}`. An initial value is
...
## WCO-C06 — Runtime and Form mapping
...
types specified in WCO.05. A credential/profile mismatch fails before I/O.
...
## WCO-C07 — Optional native executable contract
...
Protocol-specific parameters and value envelopes are defined in WCO.05 and
[WCO.08](WCO.08-native-build-and-software-evidence.md). Native protocols execute in BEAM/OTP or
...
## WCO-C08 — Telemetry and diagnostics

Retain request event `[:wotex, :coap, :request, :stop]`, duration in native
monotonic units and finite operation/result/status metadata. Add stream events
`[:wotex, :coap, :subscription, :open | :deliver | :close]` with counts and a
...
Each requirement ID in WCO.05/.11/.12 requires concrete acceptance cases with
