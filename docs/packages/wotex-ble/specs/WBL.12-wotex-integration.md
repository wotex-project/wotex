...
# WBL.12 Wotex integration and evidence contract

This is an accepted **target specification**, not implemented-profile evidence.
It makes [.10](WBL.10-software-contract.md) and
[.11](WBL.11-standalone-client-and-preservation.md) usable with the public Wotex
...
## WBL-I02 — Explicit Runtime profile factory

Add pure `profile/0` on `Wotex.BLE`, returning a `Wotex.Runtime.BindingProfile` for
...
| --- | --- | --- | --- | --- |
| `:oneshot` | `:ble` | `ble` | readproperty, writeproperty | none |
| `:gatt` | `:ble_gatt` | `ble` | readproperty, writeproperty, observeproperty, unobserveproperty, subscribeevent, unsubscribeevent | characteristic value changes |
...
A BlueZ Value change can also follow ReadValue. Stream metadata identifies the BlueZ signal source; it must not falsely label every change a verified ATT notification. Discovery and pairing are explicit native APIs.
...
{:ok, consumed} = Wotex.Runtime.ConsumedThing.new(td,
  ...
  transports: %{Wotex.Runtime.BindingProfile.id(profile) =>
    {Wotex.BLE.Transport, transport_options}},
...
## WBL-I03 — Mapping, route and value boundary
...
The device-key must exactly equal config.target; the explicit BlueZ peer/device/service/characteristic association is checked live. A URI cannot choose a bus or D-Bus object by itself.
...
## WBL-I04 — Errors, credentials and deadlines through Runtime
...
## WBL-I05 — Runtime stream ownership
...
native `{:wotex_ble, reference, event}` messages to Runtime and expect decoding.
...
Add `test/wotex/ble/runtime_integration_test.exs`, exercising real core and
...
The native implementation and bounded callback flow follow [WBL.13](WBL.13-native-backend.md). Runtime owns the final subscription process; an
