...
Read this file with [WBL.04](WBL.04-software-contract.md),
[standalone client preservation](WBL.05-standalone-client-and-preservation.md),
[Wotex integration](WBL.06-wotex-integration.md), and the existing
...
Build the explicitly enumerated client profile in WBL.04. A protocol's
...
## WBL-C02 — Input validation and bounds
...
## WBL-C03 — Session ownership and deadlines
...
## WBL-C04 — Errors, effects and retry

Keep `%Wotex.BLE.Error{}` with fields `code`, `field`, `details`,
...
## WBL-C05 — Subscription contract

Applies only to subscriptions listed in WBL.04. Native `subscribe/2` takes a
validated request map with `receiver: pid` (default caller) and returns
`{:ok, %Wotex.BLE.Subscription{pid: pid, reference: ref, generation: gen}}` only after
...
Native deliveries are `{:wotex_ble, reference, {:ok, value, metadata}}` or
`{:wotex_ble, reference, {:error, error}}`. An initial value is
...
## WBL-C06 — Runtime and Form mapping
...
types specified in WBL.04. A credential/profile mismatch fails before I/O.
...
## WBL-C07 — Native executable contract
...
Protocol-specific parameters and value envelopes are defined in WBL.04.
...
## WBL-C08 — Telemetry and diagnostics

Retain request event `[:wotex, :ble, :request, :stop]`, duration in native
monotonic units and finite operation/result/status metadata. Add stream events
`[:wotex, :ble, :subscription, :open | :deliver | :close]` with counts and a
...
## WBL-C09 — Mandatory software evidence

Each requirement ID in WBL.04/.11/.12/.13 requires concrete acceptance cases with
...
commit; it runs this package's full gate, `mix pkg wotex-ble check --no-retry`,
...
exploratory patches in the ignored `docs/tasks/local/wotex-ble/`.
