...
# WBA.06 Wotex integration and evidence contract

This is the accepted integration specification for the implemented software
profile. It makes [.10](WBA.04-software-contract.md) and
[.11](WBA.05-standalone-client-and-preservation.md) usable with the public Wotex
...
{:ok, consumed} = Wotex.Runtime.ConsumedThing.new(td,
  ...
  transports: %{Wotex.Runtime.BindingProfile.id(profile) =>
    {Wotex.BACnet.Transport, transport_options}},
...
The decimal device instance must exactly equal config.target; the explicit BACnet destination supplies its network route. An IAm is discovery evidence, not permission to change that association.
...
## WBA-I04 — Errors, credentials and deadlines through Runtime
...
## WBA-I05 — Runtime stream ownership
...
native `{:wotex_bacnet, reference, event}` messages to Runtime and expect decoding.
