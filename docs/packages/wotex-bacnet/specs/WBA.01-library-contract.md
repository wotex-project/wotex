...
# WBA.01 Software implementation rules
...
Read this file with [WBA.04](WBA.04-software-contract.md),
[standalone client preservation](WBA.05-standalone-client-and-preservation.md),
[Wotex integration](WBA.06-wotex-integration.md), and the existing
...
## WBA-C01 — Scope, API and compatibility

Build the explicitly enumerated client profile in WBA.04. A protocol's
...
## WBA-C02 — Input validation and bounds
...
## WBA-C03 — Session ownership and deadlines
...
## WBA-C04 — Errors, effects and retry

Keep `%Wotex.BACnet.Error{}` with fields `code`, `field`, `details`,
...
## WBA-C05 — Subscription contract

Applies only to subscriptions listed in WBA.04. Native `subscribe/2` takes a
...
Native deliveries are `{:wotex_bacnet, reference, {:ok, value, metadata}}` or
`{:wotex_bacnet, reference, {:error, error}}`. An initial value is
...
## WBA-C06 — Runtime and Form mapping
...
types specified in WBA.04. A credential/profile mismatch fails before I/O.
...
Retain request event `[:wotex, :bacnet, :request, :stop]`, duration in native
monotonic units and finite operation/result/status metadata. Add stream events
`[:wotex, :bacnet, :subscription, :open | :deliver | :close]` with counts and a
...
## WBA-C09 — Mandatory software evidence

Each requirement ID in WBA.04/.11/.12 requires concrete acceptance cases with
...
## WBA-C10 — Commits and completion
...
`mix pkg wotex-bacnet check --no-retry`, when the package changed. Preserve the
...
source, the package gate `mix pkg wotex-bacnet check --no-retry` passes, including
...
exploratory patches in the ignored `docs/tasks/local/wotex-bacnet/`.
