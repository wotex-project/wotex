...
Browse/BrowseNext and Call remain native APIs. Do not invent a Method Form or infer EventFilter/EventNotifier support from monitored DataChanges. Both modes enforce their explicitly configured secure policy.
...
{:ok, consumed} = Wotex.Runtime.ConsumedThing.new(td,
  ...
  transports: %{Wotex.Runtime.BindingProfile.id(profile) =>
    {Wotex.OPCUA.Transport, transport_options}},
...
The endpoint including explicit/default port and path, excluding the id query, must exactly equal config.target. Session identity also validates the configured server certificate/ApplicationUri.
...
## WOP-I04 — Errors, credentials and deadlines through Runtime
...
## WOP-I05 — Runtime stream ownership
...
native `{:wotex_opcua, reference, event}` messages to Runtime and expect decoding.
...
Add `test/wotex/opcua/runtime_integration_test.exs`, exercising real core and
...
project, not an accidental shared build. As in wotex-runtime WRT-C04, the consumer
