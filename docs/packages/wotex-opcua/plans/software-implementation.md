...
| WOP-P01 | S01/N02/N05: typed pure Variant/DataValue/ExpandedNodeId/QualifiedName/LocalizedText/reference codecs, exact signed ticks and array/null/opaque distinctions; lossless JSON integer/negative-zero IPC | typed_values_test.exs; production native value/contract checks; WOP-F01..F13 and X-F01..F16; independent complete V03 Read boundary | Accepted |
...
| WOP-P06 | S05: typed Runtime Property observation and explicit health probe, native one-shot compatibility projection, unsupported Event/credential rejection | runtime_stream_test.exs; real Runtime child-spec lifecycle tests | Open; `health_check/2` Read probe implemented with same-stack evidence; Runtime observeproperty relay with real ConsumedThing child specifications, owner death/stop/peer loss cleanup, Event and credential rejection implemented with same-stack evidence; the independent async-opcua peer executes the complete Runtime-supported scalar read set, reads and observes arrays of every supported non-null type plus a dimensioned Int16 matrix, roundtrips every writable array type, and cleans observations after explicit stop, owner death and terminal peer loss; peer loss emits one classified error and one transport_down, stops the owner, reaps the relay helpers and requires an explicitly fresh child for a replacement peer; X-F23 is bound with an injected streaming client; every value returns only after complete Variant validation; remaining Runtime profile cells remain |
| WOP-P06a | N01/N03/N04/N05: typed bounded Browse/BrowseNext/release, original Session/deadline, early-release/failure fallback and native root helpers | standalone_contract_test.exs; priv/native/browse_trace_check.c; open62541_test.exs; test/interop/native_paged_test.exs; test/interop/rust_peer_test.exs; WOP-F14..F16 and complete N boundary matrix | Open; persistent typed handles, next/release/all, original-deadline/cumulative bounds and persistent/one-shot child-list pagination pass C response fixtures and a secure same-stack multi-page wire peer; native C owns up to 64 continuation chains with per-chain bounds, a duplicate native token ends the generation, and an unconsumed continuation is released at its original deadline (failed release closes the Session) with probe and same-stack wire evidence; WOP-F14..F16 execute through the native browse trace with an injected SDK send boundary, an injected clock and the scripted host role, with mutants; at the second independent peer (async-opcua, 40 children) the server's live continuation-point count follows Browse, BrowseNext, explicit release, two concurrent chains, `max_references`/`max_pages` failures and deadline expiry back to zero, and `all/3` and child-list Browse return the children in server order; a separate Session proves the 64-live-handle client limit, local 65th-request rejection, released-slot reuse and close cleanup against the peer counter; filtered Browse independently preserves direction, exact/subtype reference selection, node-class masks, server order and every typed reference field; malformed initial Browse and BrowseNext variants cover missing and duplicate results, diagnostics, an oversized page and an overlong continuation, each closing only its owning Session, clearing successor and other peer resources, and leaving an observer usable; independent first-page and next-page status variants retain Uncertain pages, release them when complete collection fails, and preserve exact Bad result or response-header identity while distinguishing request-scoped initial failure from terminal successor cleanup; empty first and next continuation pages remain valid, complete collection advances through them, and page limits count them before releasing the live cursor; destructive variants drop the initial Browse response after recording one allocated continuation or consume BrowseNext before dropping that response; either path ends the Session, an unrelated subscription and both native helpers; a third variant consumes a valid release before dropping its response, which also ends the Session, subscription and native helpers; a fourth returns a Bad release result, which closes only the owning Session and clears its continuation, subscription and MonitoredItem while an observer stays usable; six release-fault variants cover zero and duplicate results, returned references, a returned continuation, diagnostics and a Bad response header, each closing only its owning Session and clearing its peer resources while an observer stays usable; killing another Session's owner with a live continuation and subscription clears those peer resources, reaps both native helpers and leaves the observer usable; the server's BrowseNext counter proves non-owner, foreign-Session and consumed handles fail before I/O while the valid next and release are counted exactly once; the rest of the N boundary matrix remains |
| WOP-P07 | X06/S01..S04: compiled secure peer in the admitted stack, an independent peer for the full policy/token/security matrix, async-opcua BrowseNext/release and Cancel counts, typed methods/arrays/users, subscriptions and continuation counters | test/interop/native_secure_test.exs; test/interop/native_paged_test.exs; test/interop/rust_peer_test.exs; full V01..V14 software assertions | Open; the compiled open62541 C peer passes the secure, policy/token, fault, subscription and lifecycle suites. The async-opcua Rust peer independently supplies BrowseNext/release, the 64-live-continuation boundary and Cancel counts, executes all X-F30..F38 policy/token service workflows with resource counters returning to zero, rejects every isolated X-F39..F47 fault before application traffic, returns every Runtime-supported scalar and non-null array type through reads and observations, and proves the complete V03 DataValue boundary while preserving the Session. It roundtrips every writable array type plus a dimensioned Int16 matrix and proves initial/fresh monitoring, idempotent cancellation, receiver-death isolation, terminal-once receiver overflow, ordered Republish recovery, unavailable-Republish termination, server-proven lifetime expiry and terminal server loss without reconnect/replay through both the native Session and Runtime observation paths. The remaining V01..V14 assertions remain. |

| WOP-P07a | I01..I06: exact profiles and Form/context/media selection, Result identity/metadata, complete Error/Retry table, final-owner custody and cleanup through real ConsumedThing | runtime_integration_test.exs; all wotex-integration-v1.json cases and I06 matrix | Open; one native one-shot Runtime ByteString Form Write/readback path is proven; the Runtime observation relay runs through real child specifications; Error.class and the complete I04 retry table bind WOP-I-F02..F07; static one-shot/session profiles, contentType rejection and persistent Read/Write projection bind WOP-I-F01 and pass same-stack read/write/observe; both profiles also read/write a scalar Double against the independent async-opcua peer, whose real observation releases peer and local resources after stop, owner death and terminal peer loss, with an explicitly fresh child required for a replacement peer; its ConsumedThing value slices return the complete supported scalar set, read and observe every supported non-null array type, and roundtrip every writable array type plus a dimensioned Int16 matrix; an exact-archive consumer runs the native secure workflow and the Runtime integration suite passes on both required runtimes (X-F48); the remaining I03/I05/I06 cells (Action/Call Forms stay native-only) remain |
...
WOP-P04/P05/P07 now include an independent subscription service-envelope
matrix. Eleven creation variants alter revised parameters, result cardinality,
diagnostics, item identity or Bad status after real acquisition. The client
deletes the identified subscription and keeps the Session usable. Ten delete
variants alter DeleteMonitoredItems or DeleteSubscriptions results, diagnostics
or service status after real server deletion. The client returns
`cleanup_failed`, closes only the owning Session and preserves an observer with
zero peer resources. Remaining Publish-sequence and lifecycle cells stay open.

Peer-observed service traffic binds subscription-handle ownership. A foreign
Session and forged generation fail before either delete service. The valid owner
reaches DeleteMonitoredItems and DeleteSubscriptions once each; repeated
cancellation makes no further request.

The independent peer also exposes create-service counters for the native
Session's 32-subscription bound. Thirty-two handles remain live together. The
33rd is rejected before server I/O, a confirmed delete frees one slot, and the
replacement consumes that slot without exceeding the bound.

WOP-P03/P04/P07 now include peer-counted secure-channel renewal. A one-second
server token lifetime forces renewal while a subscription is live; the same
Session then writes, receives and reads back a changed value before releasing
its peer resources. A second variant applies one Write and returns
BadSessionIdInvalid. The owner terminates with unknown effect while an observer
confirms the changed value and a single server Write request.

WOP-P04/P05/P07 also include seven independent Publish-integrity variants. An
exact duplicate is acknowledged without redelivery. Conflicting reuse, sequence
zero, a gap above the 100-message Republish bound, an unknown client handle and
StatusChange each end only the subscription and preserve the Session. A Bad
acknowledgement result closes only its Session and clears its peer resources
while an observer remains usable.

WOP-P06a now also has independent initial-page and next-page evidence for
remote ExpandedNodeIds and unknown namespace URIs. Typed Browse retains the
identity; persistent and one-shot child projection reject the lossy conversion
and release the live cursor. Remote type definitions are retained independently
and do not block child projection when the target NodeId is local. Out-of-table
namespace indexes in all three identity fields are rejected on either page with
cursor cleanup; child projection rejects only an invalid target identity.
Duplicate references on initial and next pages remain duplicated and ordered in
typed and child-list results. QualifiedName namespace, locale and UTF-8 display
text are preserved independently at both page positions. NodeClass zero stays
unspecified rather than being inferred as Variable. Null and empty
QualifiedName strings remain distinct through both typed page positions while
child projection ignores them. Null type definitions retain the null NodeId,
absent URI and zero server index without blocking child collection. An
independent long-name chain crosses the aggregate 1 MiB reference budget without
crossing the per-page frame ceiling; all three public paths release its cursor.
The filter matrix now includes both-direction Organizes results with Object and
Variable classes together, plus the all-reference-types selector over mixed
Variable and Method targets.
