...
# WMB.01 Software implementation rules
...
Read this file with [WMB.04](WMB.04-software-contract.md),
[standalone client preservation](WMB.05-standalone-client-and-preservation.md),
[Wotex integration](WMB.06-wotex-integration.md), and the existing
...
Build the explicitly enumerated client profile in WMB.04. A protocol's
...
Applies only to subscriptions listed in WMB.04. Native `subscribe/2` takes a
...
types specified in WMB.04. A credential/profile mismatch fails before I/O.
...
libmodbus peer is test-only and follows WMB.07. The following rules apply only
...
Protocol-specific parameters and value envelopes are defined in WMB.04 and
[WMB.07](WMB.07-native-build-and-software-evidence.md). Native protocols execute in BEAM/OTP or
...
[WMB.07](WMB.07-native-build-and-software-evidence.md) fixes the explicit Mix task, native
...
Each requirement ID in WMB.04/.11/.12 requires concrete acceptance cases with
...
`mix pkg wotex-modbus check --no-retry`, when the package changed. Preserve the
...
source, the package gate `mix pkg wotex-modbus check --no-retry` passes, including
...
exploratory patches in the ignored `docs/tasks/local/wotex-modbus/`.
