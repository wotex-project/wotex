...
obligation. Documentation now lives under `docs/packages/wotex-modbus/`.
...
(`WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-modbus`) and the
CI lanes now discharge `repository_green` and `archive_consumer_green`. Tags
use `wotex-modbus-v<version>`.
