...
# file, with the repository mounted read-only at its host path and the native
# cache writable (Wotex.Workspace.NativeContainer, suite.sh, bin/mix).
#
...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install -y --no-install-recommends "clang-format-${LLVM_MAJOR}" "clang-tidy-${LLVM_MAJOR}" \
