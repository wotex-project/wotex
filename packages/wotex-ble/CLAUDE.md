...
Run `WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-ble` before a
...
`docs/packages/wotex-ble/`. Mutable audit notes belong only in the ignored root
`docs/tasks/local/wotex-ble/`. No coordination daemon, worker assignments,
