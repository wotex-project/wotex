...
static int sdk(int argc, char **argv) {
    ...
    if (!strcmp(mode, "final") || !strcmp(mode, "background") || !strcmp(mode, "flood")) {
...
static struct run start(const char *mode, size_t count, int input_capacity, int output_capacity,
    ...
    CHECK(serial <= sizeof(active_guardians)/sizeof(active_guardians[0]), "fixture guardian capacity");
    active_guardians[serial-1] = run.guardian;
...
static void ready(struct run *run) {
    ...
    while (now() < deadline) {
        ...
        if (file) {
            ...
            /* NOLINTNEXTLINE(bugprone-unchecked-string-to-number-conversion,cert-err34-c): reads PIDs the check's own child wrote */
