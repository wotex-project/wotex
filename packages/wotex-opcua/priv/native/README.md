...
or writes one Value attribute asynchronously; other services remain unimplemented.
...
NamespaceArray. The production owner independently exercises open/read/write/close
...
admits secure `open`, one-at-a-time `read`/`write`, and `close`, and rejects other
...
unknown effect; there is no retry. `ipc_check.c` covers
...
control allowance; open/read/write/close responses spend credit and the BEAM owner
