...
or writes one Value attribute or calls one Method asynchronously; other services
...
NamespaceArray. The production owner independently exercises open/read/write/call/close
...
admits secure `open`, one-at-a-time `read`/`write`/`call`, and `close`, and rejects other
...
control allowance; open/read/write/call/close responses spend credit and the BEAM owner
