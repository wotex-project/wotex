...
The OPC UA profile fixes input capacity 131072, output capacity 65536 and
...
The SDK host in this OPC UA profile does not fork. The portable custody boundary
...
| --- | --- |
| WOP-G01 | Invalid limit/path/argument cases create no child; setup failure returns 126. |
| WOP-G02 | Fragmented UTF-8, NUL and all-byte payloads survive in both directions byte-for-byte. |
| WOP-G03 | Simultaneous input/output beyond either queue capacity completes in order under a reading consumer. |
| WOP-G04 | A full SDK input queue cannot hide owner EOF; a stopped/TERM-resistant SDK is reaped within its allowance. |
| WOP-G05 | A blocked stdout consumer causes finite buffering; owner EOF still ends the SDK within its allowance. |
| WOP-G06 | Output HUP/EPIPE and SDK stderr cause fixed failures without leaking bytes into protocol stdout. |
| WOP-G07 | SDK exit with a final payload larger than OUTPUT_BYTES drains its complete stream through EOF to a slow reader; a blocked reader returns 129; the direct child is reaped and owned background group members terminate. |
| WOP-G08 | Two guardians remain isolated: ending one does not signal or close the other. |
| WOP-G09 | Repeated stop events and a partly spent caller cleanup budget cannot restart or extend teardown. |
| WOP-G10 | Linux/macOS native fault builds pass; Linux ASan/UBSan output or leaked direct children fail acceptance. |
