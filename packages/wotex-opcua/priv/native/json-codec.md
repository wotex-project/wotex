...
the executable now admits secure `open`, one-at-a-time
`read`/`write`/`call`/`browse`, and `close`. Other services and complete output
buffering remain required. This format is the package's typed interprocess
protocol, not OPC UA JSON wire encoding.
