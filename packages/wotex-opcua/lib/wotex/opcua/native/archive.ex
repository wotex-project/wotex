defmodule Wotex.OPCUA.Native.Archive do
  @moduledoc """
  Verifies and extracts a bounded native source archive into a new directory.

  The explicit build supplies the digest and root from
  `Wotex.OPCUA.Native.Source`. `extract/3` reads one bounded archive snapshot,
  verifies its SHA-256 and validates every tar entry before creating output.
  Directories and regular files beneath the selected root are admitted. Links,
  duplicate paths, traversal, special files, privileged modes and excessive
  expanded sizes fail without extraction.

  The destination must be an absent absolute path beneath a caller-owned build
  workspace. Existing destinations are never reused or removed by this helper.
  Failed extraction removes only the newly created destination. Source download,
  workspace exclusivity, tool invocation and build-manifest reuse belong to the
  build owner. This module performs I/O only when called explicitly.

  ## Examples

      iex> entries = [{~c"sdk", :directory, 0, 0, 0o755, 0, 0}]
      iex> Wotex.OPCUA.Native.Archive.validate_entries(entries, "sdk")
      :ok
      iex> Wotex.OPCUA.Native.Archive.validate_entries(entries, "another-sdk")
      {:error, :invalid_archive_entry}
  """
  ...
  @typedoc "Finite source verification or extraction failure."
  @type failure ::
  ...
  @doc "Verifies an immutable archive snapshot and extracts only its permitted root."
  ...
  @doc "Checks tar metadata without writing files or trusting member-supplied paths."
  ...
  def validate_entries(entries, root) when is_list(entries) and is_binary(root) do
    if valid_root?(root) and length(entries) in 1..@entry_limit do
      result = Enum.reduce_while(entries, {0, MapSet.new()}, &entry(&1, &2, root))
  ...
  # GitHub's pinned archives contain this ignored POSIX global metadata header.
  # It is never extracted as a file and cannot supply a member path or link.
  ...
  defp unpack(bytes, destination) do
    ...
    case :erl_tar.extract({:binary, bytes}, [:compressed, {:cwd, cwd}]) do
      :ok ->
        :ok
