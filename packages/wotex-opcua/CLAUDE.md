# Wotex OPCUA Package Contract

Wotex owns W3C Web of Things terminology and core Thing Description semantics.
This normal Mix library owns OPCUA values, bounded protocol operations,
...
Run `WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-opcua` before
...
`docs/packages/wotex-opcua/`. Mutable audit notes belong only in the ignored
root `docs/tasks/local/wotex-opcua/`. No coordination daemon, worker
assignments, shared-workspace state or tool-specific project metadata.
