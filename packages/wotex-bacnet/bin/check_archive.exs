defmodule Wotex.BACnet.Check.Archive do
  ...
  def main do
    ...
    archive = "wotex_bacnet-#{Mix.Project.config()[:version]}.tar"
    temporary = Path.join(System.tmp_dir!(), "wotex-bacnet-archive.#{unique()}")
  ...
  defp verify(project_root, archive, temporary) do
    unless File.regular?(archive) do
      violation("expected current wotex_bacnet archive: #{archive}")
    end
