...
defmodule Wotex.BACnet.SoftwareCommandTest do
  @moduledoc false
  ...
  test "WBA-C09 WBA-V14 direct argv preserves combined output and child exit", context do
  ...
  test "WBA-C09 WBA-V14 timeout kills TERM-resistant owned process group", context do
    started = System.monotonic_time(:millisecond)
  ...
  test "WBA-C09 WBA-V14 successful root exit still kills background descendants", context do
    assert {output, 0} = execute(context, "background")
    assert_dead(pids(output))
  end

  test "WBA-C09 WBA-V14 stopped children remain owned until the command deadline", context do
  ...
  test "WBA-C09 WBA-V14 output flood never emits beyond the exact bound", context do
    assert {output, 125} = execute(context, "flood", output: "4097")
    assert output == String.duplicate("x", 4097)
  end

  test "WBA-C09 WBA-V14 owner EOF releases a live process group", context do
    parent = self()
  ...
  test "WBA-C09 WBA-V14 suspended consumer and full pipe cannot block native deadline", context do
    parent = self()
  ...
  test "WBA-C09 WBA-V14 terminating one command preserves another owned group", context do
  ...
  test "WBA-C09 WBA-V14 guardian TERM preserves bounded child cleanup", context do
    port = launch(context, "hang")
    assert_receive {^port, {:data, output}}, 1000
  ...
  test "WBA-C09 WBA-V14 malformed owner input is terminal", context do
    port = launch(context, "hang")
    assert_receive {^port, {:data, output}}, 1000
  ...
  test "WBA-C09 WBA-V14 advisory lease rejects overlap and releases on owner death", context do
  ...
  test "WBA-C09 WBA-V14 advisory lease refuses unrelated content and symlinks", context do
  ...
  test "WBA-C09 WBA-V14 acknowledged lease release permits immediate reacquisition", context do
