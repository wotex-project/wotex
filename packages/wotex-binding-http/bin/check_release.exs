defmodule WotexBindingHTTP.Check.ReleaseCandidate do
  @moduledoc false
  ...
  defp verify_project! do
    ...
    exact!(Keyword.get(project, :app), :wotex_binding_http, "application name")
    ...
    exact!(Keyword.get(package, :name), "wotex_binding_http", "Hex package name")
    ...
    expected_links = %{
      ...
      "GitHub" => "https://github.com/wotex-project/wotex-binding-http",
      "W3C Web of Things" => "https://www.w3.org/WoT/",
      "Wotex" => "https://wotex.io"
    }

    exact!(links, expected_links, "package links")

    files = Keyword.get(package, :files, [])

    Enum.each(@package_files, fn file ->
      unless file in files, do: violation("package allowlist is missing #{file}")
    end)
  ...
  defp verify_legal_and_security! do
    ...
    require!(notice, "Wotex HTTP Binding", "NOTICE package identity is missing")
  ...
  defp verify_documentation! do
    ...
    standards = File.read!("docs/standards-baseline.md")

    require!(
      readme,
      "release or W3C certification is implied",
      "release nonclaim is missing"
    )

    require!(standards, "does not claim Profile", "Profile nonclaim is missing")
  ...
  defp verify_lock! do
    ...
    Enum.each(lock, fn
      {_name, {:hex, _package, _version, _checksum, _managers, _dependencies, "hexpm", _outer}} ->
        :ok

      {name, value} ->
        violation("locked dependency #{name} is not an exact Hex package: #{inspect(value)}")
    end)
  ...
  defp verify_toolchain! do
    ...
    require!(
      ...
      "CI does not bootstrap all exact gate dependencies"
    )
    ...
    runtime_baseline = File.read!("docs/runtime-baseline.md")
    require!(runtime_baseline, @core_revision, "public baseline does not pin core")
    require!(runtime_baseline, @runtime_revision, "public baseline does not pin Runtime")
  end

  defp verify_gate! do
    {configuration, _binding} = Code.eval_file(".check.exs")
    ...
    Enum.each(@required_tools, fn tool ->
      unless tool in tools, do: violation("default gate is missing #{tool}")
    end)
  ...
  defp report(:ok) do
    IO.puts("release-candidate vectors: WBH-P01..WBH-P08")
...
WotexBindingHTTP.Check.ReleaseCandidate.main()
