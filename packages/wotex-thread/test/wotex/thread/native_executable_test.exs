defmodule Wotex.Thread.NativeExecutableTest do
  @moduledoc false

  use ExUnit.Case, async: true

  alias Wotex.Thread.Error
  alias Wotex.Thread.OpenThread.Executable

  setup do
    directory =
      Path.join(
        System.tmp_dir!(),
        "wotex-thread-executable-#{System.pid()}-#{System.unique_integer([:positive])}"
      )
  ...
  test "WTH-B01 admits exact executable content without starting it", context do
  ...
  test "WTH-B01 rejects mismatched, absent, linked and nonexecutable inputs", context do
  ...
  test "WTH-B01 rejects empty and oversized executables before hashing", context do
  ...
  test "WTH-B01 expires the original budget and rejects malformed selectors" do
