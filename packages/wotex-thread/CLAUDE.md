...
This normal Mix library owns Thread values, bounded protocol operations,
...
Run `WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-thread` before
...
application-free check. Native builds and the live native advisory check run
only when invoked explicitly. Specifications, plans and provenance for this
package live under `docs/packages/wotex-thread/`. Apply the shared
...
notes belong only in the ignored root `docs/tasks/local/wotex-thread/`. No
