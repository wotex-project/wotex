defmodule Wotex.Thread.OpenThread.Executable do
  @moduledoc """
  Admits one immutable OpenThread host by its explicit SHA-256 identity.

  Verification is bounded by the caller's existing monotonic deadline. It
  accepts only a nonempty executable regular file, hashes through an open file
  descriptor, and checks the file identity before and after reading as well as
  against the final path. It starts no process and retains no open descriptor.
  """
