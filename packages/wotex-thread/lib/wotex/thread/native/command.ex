defmodule Wotex.Thread.Native.Command do
  @moduledoc """
  ...
  Returned output is bounded build evidence, not a Thread result or diagnostic
  ...
  defp valid_step?(step) do
    MapSet.new(Map.keys(step)) == MapSet.new(@fields) and is_atom(step.id) and
      ...
      environment?(step.env) and integer?(step.timeout_ms, 1_800_000) and
