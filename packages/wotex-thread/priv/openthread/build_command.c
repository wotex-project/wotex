...
/*
 * Explicit build command guardian. stdin is an owner-liveness pipe. The child
 ...
 * helper contains ordinary build descendants remaining in that group; it is
...
int main(int argc, char **argv) {
    ...
    int result;
    if (argc < 6 || number(argv[1], 1800000, &timeout) ||
    ...
    if (pipe(group_ready)) { close(pipes[0]); close(pipes[1]); return 126; }
    memset(&state, 0, sizeof(state));
    state.child = fork();
    if (state.child < 0) {
        close(group_ready[0]); close(group_ready[1]);
        close(pipes[0]); close(pipes[1]); return 126;
    }
    if (state.child == 0) {
        unsigned char ready;
        ssize_t ready_size;
    ...
    if ((setpgid(state.child, state.child) && getpgid(state.child) != state.child) ||
        ...
        close(group_ready[1]); close(pipes[0]); close(pipes[1]);
        return abort_startup(state.child, (int64_t)cleanup);
    }
    close(group_ready[1]);
    close(pipes[1]);
    state.input = pipes[0];
