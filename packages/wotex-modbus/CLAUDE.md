...
Run `WOTEX_PATH_DEPS=1 mix check --no-retry` from `packages/wotex-modbus` before
...
live under `docs/packages/wotex-modbus/`. Apply the shared
...
notes belong only in the ignored root `docs/tasks/local/wotex-modbus/`. No
