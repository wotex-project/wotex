 ...
 * Opaque runtime custody derives from Wotex OPC UA ca2c4afc2fe8d4afa42b7621363c567da89ce288.
 * See custody.md for this executable's process and queue contract.
 */
...
static void group_signal(struct custody *state, int signal_number) {
    ...
    if (state->child > 0 && state->group_owned && !state->custody_lost)
...
static void child_exec(int input[2], int output[2], int diagnostic[2], int group_ready[2], char **argv) {
    struct sigaction action;
    unsigned char ready;
    ssize_t ready_size;
    close(group_ready[1]);
    ...
    close(group_ready[0]);
    if (ready_size != 1 || ready != 'G' || getpgrp() != getpid() || chdir(argv[4]) < 0) _exit(126);
...
static int abort_startup(struct custody *state) {
    /* The barrier has not released this direct child. It cannot have executed
     * a worker or spawned descendants; no process-group signal is authorized. */
    int64_t began = monotonic_ms();
    (void)kill(state->child, SIGKILL);
    if (began < 0) return 129;
    for (;;) {
        int status;
        pid_t reaped = waitpid(state->child, &status, WNOHANG);
        if (reaped == state->child) { state->child = -1; return 126; }
        if (reaped < 0 && errno != EINTR) { state->custody_lost = 1; return 129; }
        int64_t now = monotonic_ms();
        if (now < 0 || now - began >= state->cleanup_ms) return 129;
        (void)poll(NULL, 0, 1);
    }
}
