defmodule WotexLabWorkbench.Chart.Admission do
  ...
  def build(opts) when is_list(opts) do
    with true <- Keyword.keyword?(opts),
      ...
      _ -> error(:invalid_chart)
    end
  end

  def build(_), do: error(:invalid_chart)
  ...
  defp input_series(series) do
    admitted =
      Enum.reduce_while(series, MapSet.new(), fn
        ...
        %{points: points}, _ when is_list(points) and length(points) > 2_000 ->
  ...
  defp input_axis(_), do: error(:invalid_axis)
  ...
  defp text(_, path), do: error(:invalid_chart, path)
