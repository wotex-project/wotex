defmodule Wotex.Lab.Adapters.HTTP.SSE.Parser do
  ...
  defp consume(state, line, _events) when byte_size(line) > state.max_line_bytes do
    {:error,
     Error.new(:sse_line_too_long, :transport, "SSE line exceeds the byte bound", class: :protocol)}
  end
  ...
  defp field(state, "id", value) do
    if String.contains?(value, <<0>>), do: {:ok, nil, state}, else: {:ok, nil, %{state | id: value}}
  end
